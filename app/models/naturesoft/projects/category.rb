module Naturesoft::Projects
  class Category < ApplicationRecord
    ...
    def self.sort_by
      [
        ["Name","naturesoft_projects_categories.name"],
        ["Created At","naturesoft_projects_categories.created_at"]
      ]
    end
    ...
    def self.search(params)
      ...
      if params[:keyword].present?
        params[:keyword].split(" ").each do |k|
          records = records.where("LOWER(CONCAT(naturesoft_projects_categories.name,' ',naturesoft_projects_categories.description)) LIKE ?", "%#{k.strip.downcase}%") if k.strip.present?
        end
      end
      
      # for sorting
      sort_by = params[:sort_by].present? ? params[:sort_by] : "naturesoft_projects_categories.name"
